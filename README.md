...
PowerShell / WSL の両方から Excel 上のテキストボックスを読み書きし、`diff` や `grep` などの既存コマンドと組み合わせて使えます。
...
ecat txtInput | ugrep "ERROR"

ecat txtInput | usort | cate txtSorted
...
識別子は `SheetName:ShapeName` 形式です。`eopen --sheet` で既定シートを設定したあとは、シート名を省略して `ShapeName` だけでも指定できます。
...
PowerShell では `setup.ps1`、WSL では `setup-wsl.sh` を読み込むと、`eopen`, `els`, `ecat`, `cate`, `ediff`, `einfo` を関数として使えます。
...
`eopen` は Windows パスと WSL パスの両方を受け付けます。
